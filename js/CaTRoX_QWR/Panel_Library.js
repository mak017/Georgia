...
if (!window.GetProperty('SYSTEM.Chakra Checked', false) && !Date.now) {
    fb.ShowPopupMessage(
        "Use the 'Chakra' script engine if possible (requires IE9 or later) - it's faster. Select in the JScript panel configuration window (shift + right click)",
        'Library Tree'
    );
...
function userinterface() {
    ...
    var changeBrightness = function (r, g, b, percent) {
        return RGB(
            Math.min(Math.max(r + ((256 - r) * percent) / 100, 0), 255),
            Math.min(Math.max(g + ((256 - g) * percent) / 100, 0), 255),
            Math.min(Math.max(b + ((256 - b) * percent) / 100, 0), 255)
        );
    };
    this.countscol = '';
    this.expand = '';
    ...
    if (!libraryProps.showScrollbar) this.scr_w = 0;
    ...
    var get_grad = function (c, f1, f2) {
        return [
            RGB(Math.min(R(c) + f1, 255), Math.min(G(c) + f1, 255), Math.min(B(c) + f1, 255)),
            RGB(Math.max(R(c) + f2, 0), Math.max(G(c) + f2, 0), Math.max(B(c) + f2, 0))
        ];
    };
    ...
    this.get_colors = function () {
        ...
        let textCol = 0;
        if (this.dui) {
            // custom colour mapping: DUI colours can be remapped by changing the numbers (0-3)
            if (this.backcol === '') this.backcol = window.GetColourDUI(1);
            this.backcolsel = window.GetColourDUI(3);
            textCol = window.GetColourDUI(0);
        } else {
            // custom colour mapping: CUI colours can be remapped by changing the numbers (0-6)
    ...
    this.get_font = function () {
        ...
        if (!this.sizedNode) {
            // prevents node sizes from growing every time this method is called
    ...
    this.draw = function (gr) {
        try {
            if (this.bg) gr.FillSolidRect(this.x, this.y, this.w, this.h, this.backcol);
    ...
    this.focus_changed = function (ms) {
        ...
        if (k == 1) {
            ...
            timer.focus = setTimeout(function () {
                k = 0;
                timer.focus = false;
            }, ms);
            return;
...
function scrollbar() {
    ...
    var smoothness = 1 - 0.7; //window.GetProperty("ADV.Scroll: Smooth Scroll Level 0-1", 0.6561);
    ...
    this.wheel = function (step, pgkey) {
        this.check_scroll(this.scroll + step * -(libraryProps.pageScroll || pgkey ? this.rows_drawn - 1 : 3) * this.row_h);
    };
    ...
    this.metrics = function (x, y, w, h, rows_drawn, row_h) {
        ...
        this.bar_ht = Math.max(
            Math.round((this.scrollbar_height * this.rows_drawn) / this.row_count),
            Math.max(Math.min(this.scrollbar_height / 2, ui.grip_h), 5)
        );
                            ...
                            case 0:
                                gr.FillSolidRect(
                                    ...
                                    RGBA(
                                        ...
                                        ui.ct,
                                        ui.ct,
                                        !this.hover && !this.b_is_dragging ? this.alpha : this.hover && !this.b_is_dragging ? this.alpha : 192
                                    )
                                );
                                break;
                            case 1:
...
function panel_operations() {
    ...
    var view_ppt = [
        window.GetProperty(
            prefix + 'View 01: Name // Pattern',
            'View by Artist // %artist%|%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'
        ),
        window.GetProperty(
            prefix + 'View 02: Name // Pattern',
            'View by Album Artist // %album artist%|%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'
        ),
        window.GetProperty(
            prefix + 'View 03: Name // Pattern',
            'View by Album Artist ordered by Date // %album artist%|[$year($if3(%original release date%,%originaldate%,%date%)) - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'
        ),
        window.GetProperty(
            prefix + 'View 04: Name // Pattern',
            'View by Album Artist - Album // [%album artist% - ][$year($if3(%original release date%,%originaldate%,%date%)) - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'
        ),
        window.GetProperty(
            prefix + 'View 05: Name // Pattern',
            "View by Album // %album%[ '['%album artist%']']|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"
        ),
        window.GetProperty(
            prefix + 'View 06: Name // Pattern',
            'View by Genre // %<genre>%|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'
        ),
        window.GetProperty(
            prefix + 'View 07: Name // Pattern',
            'View by Year // $year($if3(%original release date%,%originaldate%,%date%))|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%'
        )
    ];
    var nm = '',
    ...
    if (!window.GetProperty('SYSTEM.View Update', false)) {
        ...
        view_ppt.push(
            window.GetProperty(
                prefix + 'View ' + padNumber(i, 2) + ': Name // Pattern',
                'View by Path // $directory_path(%path%)|%filename_ext%$nodisplay{%subsong%}'
            )
        );
    ...
    this.search_paint = function () {
        window.RepaintRect(ui.x + ui.margin, ui.y, ui.w - ui.margin, this.s_h);
    };
    ...
    this.tree_paint = function () {
        window.RepaintRect(ui.x, ui.y + paint_y, ui.w, ui.h - paint_y + 1);
    };
    ...
    this.calc_text = function () {
        ...
        for (i = 0; i < this.filt.length; i++) {
            ...
            if (!i) this.f_h = g.CalcTextHeight('String', this.filter_font);
        }
        ...
        if (this.view_by != this.folder_view) {
            ...
            for (i = 0; i < this.grp_split.length; i++) {
                ...
                if (this.multi_value[i]) {
                    ...
                    this.grp_split[i] = this.grp_split[i]
    ...
    this.on_size = function () {
        ...
        var sbar_top = !ui.scr_type ? 5 : libraryProps.searchMode ? 3 : 0,
...
function library_manager() {
    ...
    this.checkTree = function () {
        if (!this.upd && !(this.init && libraryProps.rememberTree)) return;
        this.init = false;
    ...
    this.removed_s = function (handle_list) {
        var j = handle_list.Count;
        while (j--) {
            var i = p.list.Find(handle_list[j]);
            if (i != -1) {
                p.list.RemoveById(i);
                node_s.splice(i, 1);
            }
        ...
        try {
            ...
            if (library_tree.tree.length && (!b || (b && !p.reset))) {
                ...
                for (i = 0; i < library_tree.tree.length; i++) {
                    ...
                    if (library_tree.tree[i].child.length)
                        exp.push({
                            tr: tr,
                    ...
                    if (library_tree.tree[i].sel == true)
                        sel.push({
                            tr: tr,
            ...
            if (!b || (b && !p.reset && libraryProps.rememberTree)) {
        ...
        if (!handle_list) {
            ...
            switch (n) {
                ...
                case 1:
                    ...
                    let items_b;
                    switch (
                        tree_type // check for changes to items; any change updates all
                    ) {
                        case 0:
                    ...
                    if (p.filter_by > 0 && p.s_show > 1) {
                        ...
                        var handlesInFilter = fb.CreateHandleList(),
                            newFilterItems = fb.CreateHandleList(),
                            origFilter = this.list.Clone();
                        // addns
                        try {
                            newFilterItems = fb.GetQueryItems(handle_list, p.filt[p.filter_by].type);
                        } catch (e) {}
                        origFilter.Sort();
                        newFilterItems.Sort();
                        ...
                        try {
                            handlesInFilter = fb.GetQueryItems(removeFilterItems, p.filt[p.filter_by].type);
                        } catch (e) {}
                        ...
                        if (removeFilterItems.Count || newFilterItems.Count) {
                    ...
                    if (p.s_txt) {
                        // search: check if not done
                        var newSearchItems = fb.CreateHandleList(),
                            origSearch = p.list.Clone();
                        // addns
                        try {
                            newSearchItems = fb.GetQueryItems(handle_list, p.s_txt);
                        } catch (e) {}
                        origSearch.Sort();
                        newSearchItems.Sort();
                        if (p.filter_by > 0 && p.s_show > 1) {
    ...
    this.rootNames = function (li, search) {
        ...
        switch (tree_type) {
            case 0:
                var tfo = fb.TitleFormat(p.view);
                items = tfo.EvalWithMetadbs(li);
                for (i = 0; i < total; i++) arr[i] = items[i].split('|');
                break;
            case 1:
                items = li.GetLibraryRelativePaths();
                for (i = 0; i < total; i++) arr[i] = items[i].split('\\');
                break;
        }
    };
    ...
    this.prefixes = function (n) {
        ...
        var found = false,
            j = 0,
            ln = 0;
        for (j = 0; j < swapPrefix.length; j++)
            if (n.indexOf(swapPrefix[j] + ' ') != -1) {
                found = true;
                break;
            }
        ...
        for (var i = 0; i < pr.length; i++)
            for (j = 0; j < swapPrefix.length; j++) {
                ln = swapPrefix[j].length + 1;
                if (pr[i].substr(0, ln) == swapPrefix[j] + ' ') pr[i] = pr[i].substr(ln) + ', ' + swapPrefix[j];
            }
            ...
            switch (p.multi_process) {
                case false:
                    for (l = 0; l < total; l++) {
                        ...
                        if (nU != n_o) {
                            n_o = nU;
                            this.root[i] = {name: n, sel: false, child: [], item: []};
        ...
        if (lib_update && process) {
            try {
                var exp_l = exp.length,
                    scr_l = scr.length,
                    sel_l = sel.length,
                    tree_l = library_tree.tree.length;
                for (h = 0; h < exp_l; h++) {
                    if (exp[h].tr == 0) {
                        for (j = 0; j < tree_l; j++)
                            if (library_tree.tree[j].name.toUpperCase() == exp[h].a.toUpperCase()) {
                                library_tree.branch(library_tree.tree[j]);
                                tree_l = library_tree.tree.length;
                                break;
                            }
                    } else if (exp[h].tr > 0) {
                        for (j = 0; j < tree_l; j++)
                            if (
                                library_tree.tree[j].name.toUpperCase() == exp[h].b.toUpperCase() &&
                                library_tree.tree[library_tree.tree[j].par].name.toUpperCase() == exp[h].a.toUpperCase()
                            ) {
                                library_tree.branch(library_tree.tree[j]);
                                tree_l = library_tree.tree.length;
                                break;
                ...
                for (h = 0; h < sel_l; h++) {
                    if (sel[h].tr == 0) {
                        for (j = 0; j < tree_l; j++)
                            if (library_tree.tree[j].name.toUpperCase() == sel[h].a.toUpperCase()) {
                                library_tree.tree[j].sel = true;
                                break;
                            }
                    } else if (sel[h].tr == 1) {
                        for (j = 0; j < tree_l; j++)
                            if (
                                library_tree.tree[j].name.toUpperCase() == sel[h].a.toUpperCase() &&
                                library_tree.tree[library_tree.tree[j].par].name.toUpperCase() == sel[h].b.toUpperCase()
                            ) {
                                library_tree.tree[j].sel = true;
                                break;
                            }
                        ...
                        for (j = 0; j < tree_l; j++)
                            if (
                                library_tree.tree[j].name.toUpperCase() == sel[h].a.toUpperCase() &&
                                library_tree.tree[library_tree.tree[j].par].name.toUpperCase() == sel[h].b.toUpperCase() &&
                                library_tree.tree[library_tree.tree[library_tree.tree[j].par].par].name.toUpperCase() == sel[h].c.toUpperCase()
                            ) {
                                library_tree.tree[j].sel = true;
                                break;
                            }
                ...
                while (h < scr_l && !scr_pos) {
                    if (scr[h].tr == 0) {
                        for (j = 0; j < tree_l; j++)
                            if (library_tree.tree[j].name.toUpperCase() == scr[h].a.toUpperCase()) {
                        ...
                        for (j = 0; j < tree_l; j++)
                            if (
                                library_tree.tree[j].name.toUpperCase() == scr[h].a.toUpperCase() &&
                                library_tree.tree[library_tree.tree[j].par].name.toUpperCase() == scr[h].b.toUpperCase()
                            ) {
                                sbar.check_scroll(!h ? j * ui.row_h : (j - 3) * ui.row_h);
                                scr_pos = true;
                                break;
                        ...
                        for (j = 0; j < tree_l; j++)
                            if (
                                library_tree.tree[j].name.toUpperCase() == scr[h].a.toUpperCase() &&
                                library_tree.tree[library_tree.tree[j].par].name.toUpperCase() == scr[h].b.toUpperCase() &&
                                library_tree.tree[library_tree.tree[library_tree.tree[j].par].par].name.toUpperCase() == scr[h].c.toUpperCase()
                            ) {
                                sbar.check_scroll(!h ? j * ui.row_h : (j - 3) * ui.row_h);
                                scr_pos = true;
                                break;
        ...
        switch (true) {
            case !folder:
                ...
                for (var j = 0; j < insert.Count; j++) {
                    var i = binaryInsert(insert[j]);
                    n.splice(i, 0, item_a[j].split('|'));
                    li.Insert(i, insert[j]);
                }
                break;
            case folder:
                item_a = insert.GetLibraryRelativePaths();
                for (var j = 0; j < insert.Count; j++) {
                    var i = binaryInsert(insert[j]);
                    if (i != -1) {
                        n.splice(i, 0, item_a[j].split('\\'));
        ...
        switch (true) {
            ...
            default:
                ...
                if (p.s_txt) {
                    ...
                    p.list.InsertRange(p.list.Count, newSearchItems);
                    p.sort(p.list);
                    p.sort(newSearchItems);
                    let item_a;
                    switch (tree_type) {
                        case 0:
                            var tfo = fb.TitleFormat(p.view);
                            item_a = tfo.EvalWithMetadbs(newSearchItems);
                            for (var j = 0; j < newSearchItems.Count; j++) {
                                var i = p.list.Find(newSearchItems[j]);
                                if (i != -1) node_s.splice(i, 0, item_a[j].split('|'));
                            }
                            break;
                        case 1:
                            item_a = newSearchItems.GetLibraryRelativePaths();
                            for (var j = 0; j < newSearchItems.Count; j++) {
                                var i = p.list.Find(newSearchItems[j]);
                                if (i != -1) node_s.splice(i, 0, item_a[j].split('\\'));
                            }
    ...
    this.removed = function (handle_list) {
        ...
        if (p.filter_by > 0 && libraryProps.searchMode > 1) {
            ...
            full_list_need_sort = false;
            while (j--) {
                i = full_list.Find(handle_list[j]);
...
function LibraryTree() {
    ...
    this.line_l = 0;
    this.sel_items = [];
    this.subCounts = {standard: {}, filter: {}, search: {}};
    this.tree = [];
    ...
    var sort = function (a, b) {
        return a.srt
            ...
            .localeCompare(
                b.srt.replace(/^\?/, '').replace(/(\d+)/g, function (n) {
                    return ('0000' + n).slice(-5);
                })
            );
    };
    ...
    this.clear_child = function (br) {
        br.child = [];
        this.buildTree(lib_manager.root, 0, true, true);
    };
    ...
    this.expandNodes = function (obj, isRoot) {
        ...
        if (obj.child) for (var k = 0; k < obj.child.length; k++) if (!obj.child[k].track) this.expandNodes(obj.child[k], false);
    };
    ...
    this.get_sel_items = function () {
        ...
        var i = 0;
        this.sel_items = [];
        for (i = 0; i < this.tree.length; i++) if (this.tree[i].sel) this.sel_items = this.sel_items.concat(this.sel_items, this.tree[i].item);
    ...
    this.getHandles = function (n) {
        ...
        var handle_list = fb.CreateHandleList();
        try {
            for (var i = 0; i < this.sel_items.length; i++) handle_list.Add(p.list[this.sel_items[i]]);
        } catch (e) {}
        return handle_list;
    };
    this.leave = function () {
    ...
    this.Activate_tooltip = function (ix, y) {
        if (
            tt_id == ix ||
            Math.round(ui.pad * this.tree[ix].tr + ui.margin) +
                ui.icon_w +
                (!libraryProps.tooltips || !libraryProps.fullLine ? this.tree[ix].w : this.tree[ix].tt_w) <=
                sbar.tree_w - ui.sel
        )
            return;
        if (tt_c == 2) {
            ...
            if (p.multi_swap) {
                ...
                for (k = 0; k < br_l; k++) {
                    ...
                    try {
                        ...
                        if (get) {
                            if (l < lib_manager.node[pos].length - 1) n = lib_manager.node[pos][l];
                            else n = '#get_track#';
                        }
                        isTrack = libraryProps.nodeShowTracks ? false : l < lib_manager.node[pos].length - 2 ? false : true;
                        if (n == '#get_track#') {
    ...
    this.buildTree = function (br, tr, node, full, block) {
        var br_l = br.length,
            i = 0,
            j = 0,
            l = !libraryProps.rootNode ? tr : tr - 1;
        if (p.multi_process) {
            var h = -1,
                multi = [],
                multi_cond = [],
            ...
            while (i < multi_obj.length) {
                ...
                if (n_o != nU) {
                    n_o = nU;
                    multi_cond[j] = {name: n, item: multi_obj[i].item.slice(), track: multi_obj[i].track, srt: multi_obj[i].srt};
                    j++;
                } else multi_cond[j - 1].item = multi_cond[j - 1].item.concat(multi_obj[i].item.slice());
                i++;
            }
            for (i = 0; i < br_l; i++) {
            ...
            for (i = 0; i < multi_cond.length; i++) {
                ...
                if (h != -1) {
                    br[h].item = br[h].item.concat(multi_cond[i].item.slice());
            ...
            while (i--) {
                if (i != 0 && br[i].name.toUpperCase() == br[i - 1].name.toUpperCase()) {
                    br[i - 1].item = br[i - 1].item.concat(br[i].item.slice());
                    br.splice(i, 1);
                }
            }
        }
            ...
            switch (true) {
                case l != -1 && !libraryProps.nodeShowTracks:
                    for (var r = 0; r < this.tree[j].item.length; r++) {
                        if (lib_manager.node[this.tree[j].item[r]].length == l + 1 || lib_manager.node[this.tree[j].item[r]].length == l + 2) {
                            this.tree[j].track = true;
                            break;
                        }
                    }
                    break;
                case l == 0 && lib_manager.node[this.tree[j].item[0]].length == 1:
                    this.tree[j].track = true;
                    break;
            }
            this.tree[j].count =
                !this.tree[j].track || !libraryProps.nodeShowTracks
                    ? libraryProps.nodeItemCounts == 1
                        ? ' (' + this.tree[j].item.length + ')'
                        : libraryProps.nodeItemCounts == 2
                        ? ' (' +
                          this.branchCounts(
                              this.tree[j],
                              !libraryProps.rootNode || j ? false : true,
                              true,
                              false,
                              tr +
                                  (tr > 2 ? this.tree[this.tree[pr].par].name : '') +
                                  (tr > 1 ? this.tree[pr].name : '') +
                                  (tr > 0 ? this.tree[par].name : '') +
                                  this.tree[j].name,
                              type
                          ) +
                          ')'
        ...
        if (!block) {
            if (libraryProps.rootNode && this.tree.length == 1) this.line_l = 0;
            ...
            case false:
                for (k = 0; k < br_l; k++) {
                    ...
                    try {
                        ...
                        if (n == '#get_track#') n = lib_manager.node[pos][l];
                        nU = n.toUpperCase();
                        if (n_o != nU) {
                            n_o = nU;
                            b.push({name: n});
            ...
            case true:
                ...
                for (let i = 0; i < br_l; i++) {
                    if (b[i].name.indexOf('@@') != -1) {
                        ...
                        multi_rem.push(i);
                        for (var m = 0; m < multi.length; m++)
                            multi_obj.push({name: multi[m].join('').replace(/#@#.*?#@#/g, ''), srt: multi[m].join('')});
                    }
                }
                ...
                while (i < multi_obj.length) {
                    ...
                    if (n_o != nU) {
                        n_o = nU;
                        multi_cond[j] = {name: n, srt: multi_obj[i].srt};
                        j++;
                    }
                    i++;
                }
                ...
                var full = libraryProps.rootNode && br.tr == 0 ? true : false;
        ...
        for (var j = 0; j < 4; j++) {
            ...
            if (!hot) {
                g.FillGradRect(
                    ...
                    sz - ln_w * 2,
                    91,
                    plus ? ui.iconcol_e[0] : ui.iconcol_c[0],
                    plus ? ui.iconcol_e[1] : ui.iconcol_c[1],
                    1.0
                );
            } else {
            ...
            if (plus)
                ...
                g.FillSolidRect(
                    x + (sz - sy_w) / 2,
                    y + ln_w + Math.min(ln_w, sy_w),
                    sy_w,
                    sz - ln_w * 2 - Math.min(ln_w, sy_w) * 2,
                    !hot ? ui.iconpluscol : ui.iconpluscol_h
                );
            g.FillSolidRect(
                x + ln_w + Math.min(ln_w, sy_w),
                y + (sz - sy_w) / 2,
                sz - ln_w * 2 - Math.min(ln_w, sy_w) * 2,
                sy_w,
                !hot ? ui.iconpluscol : ui.iconpluscol_h
    ...
    this.tracking = function (list, type) {
        if (type) {
            ...
            try {
                for (var i = 0; i < list.length; i++) handles.Add(p.list[list[i]]);
            } catch (e) {}
    ...
    this.load = function (list, type, add, send, def_pl, insert) {
        ...
        if (type) {
            ...
            for (i = 0; i < list.length; i++) items.Add(p.list[list[i]]);
        ...
        if (fb.IsPlaying && !add && fb.GetNowPlaying()) {
            ...
            var f_ix =
                !insert || plman.GetPlaylistFocusItemIndex(pln) == -1
                    ? plman.PlaylistItemCount(pln) - items.Count
                    : plman.GetPlaylistFocusItemIndex(pln) - items.Count;
    ...
    this.collapseAll = function () {
        ...
        for (var h = 0; h < this.tree.length; h++) {
            if (!libraryProps.rootNode || this.tree[h].tr) this.tree[h].child = [];
        }
    ...
    this.expand = function (ie, nm) {
        ...
        if (libraryProps.autoCollapse) {
            ...
            for (h = 0; h < this.tree.length; h++)
                if (!arr_contains(parent, h) && !this.tree[h].sel && (!libraryProps.rootNode || this.tree[h].tr)) this.tree[h].child = [];
            this.buildTree(lib_manager.root, 0);
        }
        ...
        var new_items = this.tree.length - start_l + nodes,
            ...
            n = Math.max(h - s, libraryProps.rootNode ? 1 : 0);
    ...
    this.draw = function (gr) {
        try {
            ...
            for (var i = start_row; i < last_row; i++) {
                ...
                if (item.bot || i === last_row - 1) {
                    ...
                    for (var drawDepth = i === last_row - 1 ? 0 : depth; drawDepth <= depth; drawDepth++) {
                        if (depthRows[drawDepth] !== undefined) {
                            ...
                            const line_row_end = i + (item.bot && drawDepth === depth ? 0.5 : 1);
                ...
                if (ui.alternate) {
                    if (i % 2 == 0) gr.FillSolidRect(ui.x, item_y + 1, sbar.stripe_w, ui.row_h - 2, ui.b1);
                    else gr.FillSolidRect(ui.x, item_y, sbar.stripe_w, ui.row_h, ui.b2);
                }
                // item selected
    ...
    this.lbtn_up = function (x, y) {
        ...
        var item = this.tree[ix],
            clickedOn =
                x < Math.round(ui.pad * item.tr) + ui.icon_w + ui.margin
                    ? ObjType.Node
                    : this.check_ix(item, x, y, false)
                    ? ObjType.Item
                    : ObjType.NoObj;
    ...
    this.dragDrop = function (x, y) {
        x -= ui.x;
        y -= ui.y; // Mordred: fix mouse offsets
        if (!lbtn_dn) return;
    ...
    this.lbtn_dblclk = function (x, y) {
        ...
        if (libraryProps.doubleClickAction || (!libraryProps.doubleClickAction && mp == 1 && !item.child.length)) {
            ...
            if (!this.gen_pl) playlistIndex = plman.ActivePlaylist;
    ...
    this.move = function (x, y) {
        ...
        if (ix !== -1) {
            m_i = ix;
            if (libraryProps.tooltips) this.Activate_tooltip(ix, y);
        }
    ...
    this.get_ix = function (x, y, simple, type) {
        ...
        if (
            ...
            y > p.s_h &&
            y < p.s_h + p.sp &&
            this.check_ix(this.tree[ix], x + ui.x, y + ui.y, type)
        )
            return ix;
        else return -1;
    };
    ...
    this.check_ix = function (br, x, y, type, print) {
        ...
        return type
            ? x >= Math.round(ui.pad * br.tr + ui.margin) && x < Math.round(ui.pad * br.tr + ui.margin) + br.w + ui.icon_w
            : x >= Math.round(ui.pad * br.tr + ui.margin) + ui.icon_w &&
                  x < Math.min(Math.round(ui.pad * br.tr + ui.margin) + ui.icon_w + br.w, sbar.tree_w);
    };
    ...
    this.on_key_down = function (vkey) {
        ...
        switch (vkey) {
            ...
            case v.pgUp:
                ...
                p.pos = Math.max(!libraryProps.rootNode ? 0 : 1, p.pos);
                sbar.wheel(1, true);
            ...
            case v.home:
                if (this.tree.length == 0) break;
                p.pos = !libraryProps.rootNode ? 0 : 1;
                sbar.check_scroll(0);
                this.get_selection(this.tree[p.pos].ix);
                p.tree_paint();
                if (libraryProps.autoFill) this.load(this.sel_items, true, false, false, this.gen_pl, false);
...
function searchLibrary() {
    ...
    var calc_text = function () {
        var im = gdi.CreateImage(1, 1),
            g = im.GetGraphics();
        txt_w = g.CalcTextWidth(p.s_txt.substr(offsetChars), ui.font);
        im.ReleaseGraphics(g);
    };
    ...
    var drawsel = function (gr) {
        ...
        gr.DrawLine(
            Math.min(p.s_x + get_cursor_x(selStart), clamp),
            cursor_y,
            Math.min(p.s_x + get_cursor_x(selEnd), clamp),
            cursor_y,
            ui.row_h - 3,
            selcol
        );
    ...
    var get_cursor_pos = function (x) {
        var im = gdi.CreateImage(1, 1),
            ...
            nx = x - p.s_x,
            pos = 0;
        for (i = offsetChars; i < p.s_txt.length; i++) {
    ...
    this.lbtn_up = function (x, y) {
        if (selStart != selEnd) timer.reset(timer.search_cursor, timer.search_cursori);
        lbtn_dn = false;
    };
    this.move = function (x, y) {
        ...
        if (t < selStart) {
            if (t < selEnd) {
                if (t_x < p.s_x) if (offsetChars > 0) offsetChars--;
            } else if (t > selEnd) {
    ...
    this.rbtn_up = function (x, y) {
        men.search_menu(x, y, selStart, selEnd, doc.parentWindow.clipboardData.getData('text') ? true : false);
    };
    ...
    this.on_char = function (code, force) {
        ...
        switch (code) {
            case v.enter:
                ...
                try {
                    items = fb.GetQueryItems(lib_manager.list, p.s_txt);
                } catch (e) {}
                library_tree.load(items, false, false, false, library_tree.gen_pl, false);
                break;
            case v.redo:
                lg.push(p.s_txt);
                if (lg.length > 30) lg.shift();
            ...
            case v.selAll:
                selStart = 0;
                selEnd = p.s_txt.length;
                break;
            case v.copy:
                if (selStart != selEnd) doc.parentWindow.clipboardData.setData('text', p.s_txt.substring(selStart, selEnd));
                break;
            case v.cut:
                if (selStart != selEnd) doc.parentWindow.clipboardData.setData('text', p.s_txt.substring(selStart, selEnd));
            case v.back:
                ...
                if (selStart == selEnd) {
                    if (cx > 0) {
                        ...
                        if (offsetChars > 0) offsetChars--;
                        cx--;
                    }
                } else {
                    if (selEnd - selStart == p.s_txt.length) {
                        p.s_txt = '';
                        cx = 0;
                    } else {
                        if (selStart > 0) {
                            var st = selStart,
                                en = selEnd;
                            selStart = Math.min(st, en);
                            selEnd = Math.max(st, en);
                            p.s_txt = p.s_txt.substring(0, selStart) + p.s_txt.substring(selEnd, p.s_txt.length);
                            cx = selStart;
                        } else {
                            p.s_txt = p.s_txt.substring(selEnd, p.s_txt.length);
                            cx = selStart;
                        }
                    }
                }
                calc_text();
                offsetChars = offsetChars >= selEnd - selStart ? offsetChars - selEnd + selStart : 0;
                selStart = cx;
                selEnd = selStart;
                break;
            case 'delete':
            ...
            default:
                ...
                if (selStart == selEnd) {
                    ...
                    p.s_txt = p.s_txt.substring(0, selStart) + text + p.s_txt.substring(selEnd);
                    calc_text();
                    offsetChars = offsetChars >= selEnd - selStart ? offsetChars - selEnd + selStart : 0;
                    cx = selStart + text.length;
                    selStart = cx;
                    selEnd = selStart;
                } else {
                    p.s_txt = p.s_txt.substring(selStart) + text + p.s_txt.substring(0, selEnd);
                    calc_text();
                    offsetChars = offsetChars < selEnd - selStart ? offsetChars - selEnd + selStart : 0;
                    cx = selEnd + text.length;
                    selStart = cx;
                    selEnd = selStart;
                }
                break;
        ...
        timer.search = setTimeout(function () {
            ...
            if (libraryProps.searchAutoExpand) {
                ...
                sbar.set_rows(library_tree.tree.length);
                p.tree_paint();
                lib_manager.treeState(false, libraryProps.rememberTree);
            }
            timer.search = false;
    ...
    this.on_key_down = function (vkey) {
        ...
        switch (vkey) {
            ...
            case v.home:
            case v.end:
                if (vkey == v.home) offsetChars = selStart = selEnd = cx = 0;
                else selStart = selEnd = cx = p.s_txt.length;
                p.s_cursor = true;
                timer.reset(timer.search_cursor, timer.search_cursori);
                timer.search_cursor = setInterval(function () {
                    p.s_cursor = !p.s_cursor;
    ...
    this.on_char = function (code) {
        ...
        if (!p.s_search) {
            ...
            timer.jsearch = setTimeout(function () {
                for (i = 0; i < library_tree.tree.length; i++) {
                    if (
                        library_tree.tree[i].name != p.baseName &&
                        library_tree.tree[i].name.substring(0, jSearch.length).toLowerCase() == jSearch.toLowerCase()
                    ) {
                        found = true;
                        pos = i;
                        ...
                        if (libraryProps.autoFill) library_tree.get_sel_items();
                        lib_manager.treeState(false, libraryProps.rememberTree);
                        break;
                    }
                }
            ...
            timer.clear_jsearch = setTimeout(function () {
                if (found && libraryProps.autoFill) library_tree.load(library_tree.sel_items, true, false, false, library_tree.gen_pl, false);
                jSearch = '';
                window.RepaintRect(ui.x, j_y, ui.w, j_h + 1);
...
function button_manager() {
    ...
    var browser = function (c) {
        if (!_.runCmd(c)) fb.ShowPopupMessage('Unable to launch your default browser.', 'Library Tree');
    };
    ...
    this.lbtn_up = function (x, y) {
        this.Dn = false;
        if (libraryProps.showScrollbar) for (let j = 0; j < b3.length; j++) this.btns[b3[j]].down = false;
        if (!this.b) return false;
        if (libraryProps.showScrollbar)
            for (let j = 0; j < b3.length; j++)
    ...
    this.move = function (x, y) {
        if (sbar.timer_but) {
            if (
                (this.btns['scrollUp'].down || this.btns['scrollDn'].down) &&
                !this.btns['scrollUp'].trace(x, y) &&
                !this.btns['scrollDn'].trace(x, y)
            ) {
        ...
        for (i in this.btns) {
            if (
                (libraryProps.searchMode == 1 || (libraryProps.searchMode > 1 && !p.s_txt)) &&
                i == 's_img' &&
                (!this.Dn || this.Dn == 's_img') &&
                this.btns[i].trace(x, y)
            ) {
                b = i;
                hand = true;
            }
            if (libraryProps.searchMode == 1 && i == 'cross1' && (!this.Dn || this.Dn == 'cross1') && this.btns[i].trace(x, y)) {
                b = i;
                hand = true;
            }
            if (libraryProps.searchMode > 1 && p.s_txt && i == 'cross2' && (!this.Dn || this.Dn == 'cross2') && this.btns[i].trace(x, y)) {
                b = i;
                hand = true;
            }
            if (libraryProps.searchMode > 1 && i == 'filter' && (!this.Dn || this.Dn == 'filter') && this.btns[i].trace(x, y)) {
                b = i;
                hand = true;
            }
            if (libraryProps.showScrollbar && sbar.scrollable_lines > 0)
                for (let j = 0; j < b3.length; j++) if (i == b3[j] && (!this.Dn || this.Dn == b3[j]) && this.btns[i].trace(x, y)) b = i;
        }
    ...
    var btn = function (x, y, w, h, type, ft, txt, stat, img_src, down, l_dn, l_up, tooltext) {
        this.draw = function (gr) {
            switch (type) {
                ...
                case 4:
                    gr.DrawLine(
                        Math.round(this.x + bh * 0.67),
                        Math.round(this.y + bh * 0.67),
                        Math.round(this.x + bh * 0.27),
                        Math.round(this.y + bh * 0.27),
                        Math.round(bh / 10),
                        RGBA(136, 136, 136, this.img)
                    );
                    gr.DrawLine(
                        Math.round(this.x + bh * 0.67),
                        Math.round(this.y + bh * 0.27),
    ...
    this.refresh = function (upd) {
            ...
            switch (ui.scr_type) {
                ...
                default:
                    this.btns.scrollUp = new btn(
                        ...
                        scrollUp_y,
                        ui.scr_but_w,
                        {normal: scr[0], hover: scr[1], down: scr[2]},
                        false,
                        function () {
                            sbar.but(1);
                        },
        ...
        if (libraryProps.searchMode) {
            ...
            this.btns.filter = new btn(
                ...
                '▼',
                '',
                {normal: ui.txt_box & 0x99ffffff, hover: ui.txt_box & 0xe4ffffff},
                false,
                '',
                function () {
                    men.button(p.filter_x1, p.s_h);
                    but.refresh(true);
                },
                'Filter'
            );
...
function menu_object() {
    ...
    var InputBox = function (prompt, title, msg) {
        ...
        if (tmp.length == 254)
            fb.ShowPopupMessage('Your entry is too long and will be truncated.\n\nEntries are limited to 254 characters.', 'Library Tree');
        return tmp.trim();
    };
    var proceed = function (length) {
        var ns = InputBox(
            'Create m-TAGS in selected music folders\n\nProceed?\n\nm-TAGS creator settings apply',
            'Create m-TAGS in Selected Folders',
            'Create ' + length + ' m-TAGS' + (length ? '' : ': NO FOLDERS SELECTED')
        );
        if (!ns) return false;
        return true;
    };
    ...
    this.PlaylistTypeMenu = function (Menu, StartIndex) {
        ...
        for (i = 0; i < 6; i++) {
            ...
            Menu.AppendMenuItem(
                (i < 3 && !plman.IsPlaylistLocked(plman.ActivePlaylist)) || i == 3 || i == 4 || i == 5 ? MF_STRING : MF_GRAYED,
                idx,
                n[i]
            );
    ...
    this.TagTypeMenu = function (Menu, StartIndex) {
        ...
        Menu.AppendMenuItem(
            mtags_installed && p.view.replace(/^\s+/, '') == '$directory_path(%path%)|%filename_ext%' ? MF_STRING : MF_GRAYED,
            Index,
            'Create m-TAGS...' +
                (mtags_installed
                    ? p.view.replace(/^\s+/, '').toLowerCase() == '$directory_path(%path%)|%filename_ext%'
                        ? ''
                        : ' N/A Requires View by Path // $directory_path(%path%)|%filename_ext%$nodisplay{%subsong%}'
    ...
    this.rbtn_up = function (x, y) {
        ...
        if (
            y < p.s_h + p.sp + ui.y &&
            library_tree.tree.length > ix &&
            ix != -1 &&
            ((x < Math.round(ui.pad * item.tr) + ui.icon_w + ui.margin + ui.x && (!item.track || (libraryProps.rootNode && item.tr == 0))) ||
                ...
                case 'Playlist':
                    switch (i) {
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            library_tree.expand(ix, nm);
                            break;
                        default:
                            // Insert or Add to Current Playlist
                    ...
                    break;
                case 'Tag':
                    var r = !libraryProps.rootNode ? library_tree.tree[ix].tr : library_tree.tree[ix].tr - 1,
                        list = [];
                    if ((libraryProps.rootNode && !ix) || !r) library_tree.tree[ix].sel = true;
                    if (libraryProps.rootNode && library_tree.tree[0].sel)
                        for (var j = 0; j < library_tree.tree.length; j++) if (library_tree.tree[j].tr == 1) library_tree.tree[j].sel = true;
                    p.tree_paint();
                    for (j = 0; j < library_tree.tree.length; j++)
                        if (library_tree.tree[j].tr == (libraryProps.rootNode ? 1 : 0) && library_tree.tree[j].sel)
                            list.push(library_tree.tree[j].name);
                    if (!proceed(list.length)) break;
                    p.syncType = 1;
                    for (j = 0; j < list.length; j++) _.runCmd('"' + fb.FoobarPath + '\\foobar2000.exe"' + ' /m-TAGS "' + list[j] + '"');
...
function timers() {
    ...
    this.lib = function () {
        setTimeout(function () {
            if ((ui.w < 1 || !window.IsVisible) && libraryProps.rememberTree) lib_manager.init = true;
            lib_manager.get_library();
...
function LibraryCallbacks() {
    ...
    this.on_focus = function (is_focused) {
        ...
        library_tree.on_focus(is_focused);
    };
    ...
    this.on_library_items_added = function (handle_list) {
        if (p.syncType) return;
        lib_manager.treeState(false, 2, handle_list, 0);
    };
    this.on_library_items_removed = function (handle_list) {
        if (p.syncType) return;
        lib_manager.treeState(false, 2, handle_list, 2);
    };
    this.on_library_items_changed = function (handle_list) {
        if (p.syncType) return;
    ...
    this.on_mouse_lbtn_dblclk = function (x, y, m) {
        ...
        if (libraryProps.searchMode) sL.on_mouse_lbtn_dblclk(x, y, m);
    };
    ...
    this.on_mouse_leave = function () {
        if (libraryProps.searchMode || libraryProps.showScrollbar) but.leave();
        sbar.leave();
        library_tree.leave();
    };
    this.on_mouse_mbtn_up = function (x, y) {
        library_tree.mbtn_up(x, y);
    };
    this.on_mouse_move = function (x, y, m) {
        if (p.m_x == x && p.m_y == y) return;
        if (libraryProps.searchMode || libraryProps.showScrollbar) but.move(x, y);
        if (libraryProps.searchMode) sL.move(x, y);
        library_tree.move(x, y);
        library_tree.dragDrop(x, y);
        sbar.move(x, y);
        p.m_x = x;
        p.m_y = y;
    };
    this.on_mouse_rbtn_up = function (x, y) {
        if (y < p.s_h && x > p.s_x && x < p.s_x + p.s_w2) {
            if (libraryProps.searchMode) sL.rbtn_up(x, y);
            return true;
        } else {
            men.rbtn_up(x, y);
            return true;
    ...
    this.on_playlist_items_added = function (pn) {
        if (pn == plman.ActivePlaylist) ui.upd_handle_list = true;
        on_item_focus_change();
    };
    this.on_playlist_items_removed = function (pn) {
        if (pn == plman.ActivePlaylist) ui.upd_handle_list = true;
        on_item_focus_change();
    };
    this.on_playlist_items_reordered = function (pn) {
        if (pn == plman.ActivePlaylist) ui.upd_handle_list = true;
    };
