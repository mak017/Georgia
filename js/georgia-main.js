...
const fontThin = 'Fira Sans UltraLight';
const fontLight = 'Fira Sans Light';
const fontRegular = 'Fira Sans';
const fontBold = 'Fira Sans SemiBold';
const fontBoldMono = 'Fira Mono Medium';
const fontLightAlternate = 'Fira Sans Light';
...
function createFonts() {
    ...
    g_tooltip.SetFont('Fira Sans', scaleForDisplay(15));
...
function draw_ui(gr) {
    ...
    if (fb.IsPlaying && (albumart || !cdart) && (!displayLibrary || !settings.hidePanelBgWhenCollapsed)) {
        gr.SetSmoothingMode(SmoothingMode.None);
        gr.FillSolidRect(0, albumart_size.y, albumart_size.x + albumart_size.w, gridHeight, col.primary);
    ...
    if ((!displayLibrary || (!albumart && noArtwork)) && fb.IsPlaying) {
        ...
        if (gridSpace > 120) {
            ...
            if (str.album_artist && str.album_artist !== str.artist) {
                let font_array = [ft.album_artist_lrg, ft.album_artist_med, ft.album_artist_sml];
                var ft_album_artist = chooseFontForWidth(gr, text_width, str.album_artist, font_array, 2);
                txtRec = gr.MeasureString(str.album_artist, ft_album_artist, 0, 0, text_width, wh);
                ...
                if (value.length) {
                    ...
                    if (top + txtRec.Height < albumart_size.y + gridHeight) {
                        ...
                        if (playCountVerifiedByLastFm && showLastFmImage) {
                            ...
                            if (txtRec.Width + scaleForDisplay(10) + Math.round(lastFmLogo.Width * heightRatio) < col2_width) {
                                gr.DrawImage(
                                    lastFmLogo,
    ...
    if ((fb.IsPlaying && !displayLibrary) || (!albumart && !cdart && noArtwork)) {
        ...
        if (recordLabels.length > 0) {
            const labels =
                brightBackground && !pref.labelArtOnBg ? (recordLabelsInverted.length ? recordLabelsInverted : recordLabels) : recordLabels;
...
function onOptionsMenu(x, y) {
    ...
    configMenu.addItem('Reset configuartion file', false, () => {
...
function on_playback_stop(reason) {
    ...
    if (albumArtTimeout) clearTimeout(albumArtTimeout);
...
function ResizeArtwork(resetCDPosition) {
    ...
    if (albumart && albumart.Width && albumart.Height) {
        ...
        var album_scale = Math.min(
            ...
            (wh - geo.top_art_spacing - lowerSpace - scaleForDisplay(10)) / albumart.Height
        );
        // console.log(`album_scale`, album_scale);
        // console.log(`0.47 * ww`, (0.47 * ww) / albumart.Width);
        // console.log(`0.2905581637976004 * ww`, (0.2905581637976004 * ww) / albumart.Width);
        // console.log(
        ...
        if (displayPlaylist || displayLibrary) {
            xCenter = 0.288 * ww;
            // console.log(`xCenter if1`, xCenter);
        } else if (ww / wh < 1.4) {
...
function createButtonObjects(ww, wh) {
    ...
    if (transport.enableTransportControls) {
        let count = 4 + (transport.showRandom ? 1 : 0) + (transport.showVolume ? 1 : 0) + (transport.showReload ? 1 : 0);

        const y = transport.displayBelowArtwork
            ? wh - geo.lower_bar_h - scaleForDisplay(10) - buttonSize
