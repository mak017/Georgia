...
function draw_ui(gr) {
    ...
    if ((!displayLibrary || (!albumart && noArtwork)) && fb.IsPlaying) {
        ...
        if (str.trackInfo) {
            ...
            gr.DrawString(
                ...
                ft.track_info,
                col.artist,
                ww - textLeft * 2 - infoWidth,
                geo.top_bg_h - trackInfoHeight - 15,
                infoWidth,
                trackInfoHeight,
        ...
        if (str.year) {
            ...
            gr.DrawString(
                ...
                col.artist,
                ww - textLeft * 2 - infoWidth,
                geo.top_bg_h - trackInfoHeight - height - 20,
                infoWidth,
                height,
                StringFormat(StringAlignment.Far)
            );
    ...
    if (fb.IsPlaying) {
        if (albumart && albumart_scaled) {
            ...
            if (!pref.darkMode) {
                if (!shadow_image) {
                    // when switching views, the drop shadow won't get created initially which is very jarring when it suddenly appears later, so create it if we don't have it.
                    createDropShadow();
                }
                shadow_image &&
                    gr.DrawImage(
            ...
            if (cdart && !rotatedCD && !displayLibrary && pref.display_cdart) {
                CreateRotatedCDImage();
            }
            if (!pref.cdart_ontop || displayLyrics) {
                if (rotatedCD && !displayLibrary && pref.display_cdart && cdart_size.y > albumart_size.y && cdart_size.h < albumart_size.h) {
            ...
            if (0 && str.album) {
                ...
                if (drawSubtitle) {
                    gr.DrawString(
                        ...
                        ft_subtitle,
                        col.info_text,
                        textLeft + Math.ceil(txtRec.width),
                        top,
                        text_width - Math.ceil(txtRec.width),
                        height,
                        g_string_format.trim_ellipsis_word
                    );
                }
    ...
    if ((fb.IsPlaying && displayPlaylist && !displayLibrary) || (!albumart && !cdart && noArtwork)) {
        ...
        if (recordLabels.length > 0) {
            ...
            if (!lastLeftEdge) {
                ...
                if (albumart) {
                    if (cdart && pref.display_cdart) {
                        ...
                        leftEdge = Math.round(
                            Math.max(albumart_size.x + albumart_size.w + leftEdgeWidth + leftEdgeGap, ww * 0.975 - totalLabelWidth + 1)
                        );
                    }
                } else {
            ...
            if (labelAreaWidth >= (is_4k ? 100 : 50)) {
                ...
                allLabelsWidth = Math.max(
                    Math.min(Math.round((labelAreaWidth - labelSpacing * (labels.length - 1)) / labels.length), maxLabelWidth),
                    50
    ...
    if (ww > 600) {
        if (str.disc != '') width = gr.CalcTextWidth(str.disc + '   ' + str.time + '   ' + str.length, ft.lower_bar);
        else width = gr.CalcTextWidth(' ' + str.time + '   ' + str.length, ft.lower_bar);
    } else {
        width = 0;
    }
    ...
    if (str.original_artist && width < 0.95 * ww) {
        ...
        gr.DrawString(
            ...
            col.now_playing,
            pbLeft + trackNumWidth + titleMeasurements.Width + h_spacing,
...
function show_lyrics(gr, tab, posy) {
    ...
    if (showDebugTiming) show_lyricsTime = fb.CreateProfiler('show_lyrics');
    gr.SetTextRenderingHint(TextRenderingHint.AntiAliasGridFit);
    var g_txt_align = cc_stringformat;

    if (
        dividerImg &&
        dividerImg.width < albumart_size.w - 10 &&
        posy - divider_spacing - dividerImg.height >= albumart_size.y + pref.lyrics_h_padding
    ) {
        gr.FillRoundRect(
            albumart_size.x + Math.floor(albumart_size.w * 0.2) - 1,
            posy - divider_spacing - divider_height + 1,
            Math.floor(albumart_size.w * 0.6),
            divider_height,
            divider_height / 2,
            divider_height / 2,
            col.darkAccent
        ...
        gr.FillRoundRect(
            albumart_size.x + Math.floor(albumart_size.w * 0.2),
            posy - divider_spacing - divider_height,
            Math.floor(albumart_size.w * 0.6),
            divider_height,
            divider_height / 2,
            divider_height / 2,
        ...
        if (posy >= albumart_size.y + pref.lyrics_h_padding && posy < albumart_size.h - pref.lyrics_h_padding) {
            ...
            pref.lyrics_glow &&
                gr.DrawString(
                    ...
                    ft.lyrics,
                    g_txt_shadowcolor,
                    albumart_size.x + (albumart_size.w - lyricsWidth) / 2 - 1,
                    posy,
                    lyricsWidth,
                    lineHeight,
                    g_txt_align
            ...
            pref.lyrics_glow &&
                gr.DrawString(
                    ...
                    ft.lyrics,
                    g_txt_shadowcolor,
                    albumart_size.x + (albumart_size.w - lyricsWidth) / 2,
                    posy - 1,
                    lyricsWidth,
                    lineHeight,
                    g_txt_align
                );
            pref.lyrics_text_shadow &&
                gr.DrawString(
                    tab[i].text,
                    ft.lyrics,
                    g_txt_shadowcolor,
                    albumart_size.x + (albumart_size.w - lyricsWidth) / 2 + 2,
                    posy + 2,
                    lyricsWidth,
                    lineHeight,
                    g_txt_align
    ...
    if (showDebugTiming) show_lyricsTime.Print();
}
...
function on_size() {
    ...
    if (albumart) midpoint = Math.ceil(albumart_size.y + pref.lyrics_line_height + albumart_size.h / 2);
    else midpoint = Math.ceil((wh - geo.lower_bar_h + pref.lyrics_line_height) / 2);
    if (displayLyrics) {
        refresh_lyrics();
    }
...
function on_playback_new_track(metadb) {
    ...
    if (bandStr) {
        ...
        var path = testBandLogo(pref.logo_hq, bandStr, true) || testBandLogo(pref.logo_color, bandStr, true); // try 800x310 white // try 800x310 color
        if (path) {
...
function on_playback_stop(reason) {
    ...
    if (globTimer) window.ClearTimeout(globTimer);
    ...
    while (flagImgs.length > 0) disposeImg(flagImgs.pop());
...
function createDropShadow() {
    ...
    if ((albumart && albumart_size.w > 0) || (cdart && pref.display_cdart && cdart_size.w > 0)) {
        ...
        else shadow_image = gdi.CreateImage(albumart_size.x + albumart_size.w + 2 * geo.aa_shadow, albumart_size.h + 2 * geo.aa_shadow);
        if (shadow_image) {
...
function SetProgressBarRefresh() {
    ...
    if (fb.PlaybackLength > 0) {
        if (pref.freq_update) {
            ...
            while (
                t_interval > 500 // we want even multiples of the base t_interval, so that the progress bar always updates as smoothly as possible
            )
                t_interval = Math.floor(t_interval / 2);
            while (t_interval < 25) t_interval *= 2;
        } else {
            t_interval = 333; // for slow computers, only update 3x a second
        }

        if (showDebugTiming) console.log('Progress bar will update every ' + t_interval + 'ms or ' + 1000 / t_interval + ' times per second.');
...
function CreateRotatedCDImage() {
    ...
    if (pref.display_cdart) {
        ...
        if (cdart && cdart_size.w > 0) {
            ...
            trackNum =
                parseInt(
                    fb.TitleFormat('$num($if(' + tf.vinyl_tracknum + ',$sub($mul(' + tf.vinyl_tracknum + ',2),1),$if2(%tracknumber%,1)),1)').Eval()
                ) - 1;
...
function ResizeArtwork(resetCDPosition) {
    ...
    if (albumart && albumart.Width && albumart.Height) {
        ...
        albumart_size.x = Math.floor(xCenter - 0.25 * albumart_size.w); // left
    ...
    if (cdart) {
        if (hasArtwork) {
            if (resetCDPosition) {
                ...
                else cdart_size.x = Math.floor(albumart_size.x + albumart_size.w - (albumart_size.h - 4) * (1 - pref.cdart_amount));
                ...
                cdart_size.x = Math.max(
                    cdart_size.x,
                    Math.floor(
                        Math.min(0.99 * ww - albumart_size.h, albumart_size.x + albumart_size.w - (albumart_size.h - 4) * (1 - pref.cdart_amount))
                    )
                );
            ...
            var album_scale = Math.min(
                ...
                (wh - geo.top_art_spacing - geo.lower_bar_h - 32) / cdart.Height
            );
            if (displayPlaylist || displayLibrary) {
                xCenter = 0.25 * ww;
            } else if (ww / wh < 1.4) {
...
function fetchNewArtwork(metadb) {
    ...
    if (pref.display_cdart && !isStreaming) {
        ...
        if (disc_art_exists) {
            var temp_cdart;
            if (loadFromCache) temp_cdart = art_cache.getImage(cdartPath);
            if (temp_cdart) {
                disposeCDImg(cdart);
                cdart = temp_cdart;
                ResizeArtwork(true);
...
function createButtonObjects(ww, wh) {
    ...
    if (pref.show_transport) {
        ...
        btns[++count] = new Button(
            ...
            h,
            'Play/Pause',
            fb.IsPlaying ? (fb.IsPaused ? btnImg.Play : btnImg.Pause) : btnImg.Play,
            'Play'
        );
    ...
    try {
        ...
        console.log(
            'ATTENTION: Buttons could not be created, most likely because the images were not found in "' +
                ...
                '"'
        ); //.Eval().substring(0, str.lastIndexOf("/")));
    ...
    for (var i in btn) {
        ...
        for (var s = 0; s <= 2; s++) {
            ...
            if (btn[i].type == 'menu') {
                ...
                g.DrawImage(
                    btn[i].ico,
                    Math.round((w - btn[i].ico.width) / 2),
                    Math.round((h - btn[i].ico.height) / 2),
                    btn[i].ico.width,
                    btn[i].ico.height,
                    0,
                    0,
